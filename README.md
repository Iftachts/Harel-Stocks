...
`get_item`, `explain`, `sources`, `calendar`, `universe`, `health`.
...
### לבדוק את המערכת — לא להאמין לה

סוחר יומי לא יכול לסחור על מספר שמכונה נתנה לו בלי לראות מאיפה הוא בא. לכל ידיעה
בטרמינל יש קישור **`why?`** שפותח את דף הראיות שלה:

* **איזו שאילתה מצאה אותה** — לא "Google News" אלא הקווארי המדויק (למשל `KEN rival products`)
* **כמה המקור שווה, במילים** — "אגרגטור: כתיבה מחדש של דיווח של מישהו אחר", לא רק `0.60`
* **זמן פרסום בשלושה אזורי זמן** ואם זה לפני או אחרי הפעמון
* **כמה זמן לקח לנו לראות את זה** — פיגור של 4 שעות הוא כבר לא חדשות שאפשר לסחור עליהן
* **למה זה תויג לטיקר הזה** — הכלל שנתפס בפועל, למשל *הידיעה מזכירה "Hughes JUPITER", מוצר מתחרה של GILT*
* **הניקוד צעד-אחר-צעד** — בסיס, כל מכפיל, כל בונוס, כל תקרה
* **מי עוד נשא את הסיפור**, עם קישורים וזמנים
* **מאיפה הגיע המחיר** — איזה ספק, לפני כמה דקות נלכד, והחיסור עצמו שמאחורי האחוז

בתחתית הדף יש קופסת **Check it yourself**: קישורים ישירים למסמך המקורי, להגשות
ב־SEC EDGAR, לדיווחים המיידיים במאיה, ולמסך ציטוט. **אף אחד מהם הוא לא אנחנו.**
אם המקור אומר משהו אחר ממה שכתוב אצלנו — המקור צודק.

אותו דבר גם מחוץ לדפדפן: `harel explain <id>`, וגם ככלי MCP לסוכן.

`/sources` (ו־`harel sources`) עונה על השאלה השנייה: **האם המערכת בכלל הסתכלה?** —
כל מקור, האמון שלו, כמה פריטים החזיר במעבר האחרון, ומתי הצליח בפעם האחרונה.
...
**התראות טייפ.** תנועה שברחה מהסקטור שלה בלי שום סיפור מאחוריה מקבלת
`UNEXPLAINED RELATIVE MOVE` בראש הדף. זו ההתראה היחידה שלא נבנית מחדשות — פיד
חדשות יכול להתריע רק כשמישהו מפרסם, והימים החשובים הם בדיוק אלה שבהם אף אחד לא
מפרסם. היא מנוסחת כשאלה, לא כממצא, ואומרת מה חיפשנו ולא מצאנו.

**כתבות שנכתבו *בגלל* שהמניה זזה** ("Shares Gap Up", "Gains as Investors Weigh",
וסיכומי המסחר בעברית) הן תוצאה, לא סיבה. הן לעולם לא יוצגו כ־driver של התנועה
שהן מתארות, אבל כן יוצגו מתויגות `post-move commentary` — כדי שיהיה ברור שסיווגנו
אותן ולא פספסנו.
